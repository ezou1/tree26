...
interface DataPanelProps {
  ...
  onToggleDrugSelection?: (drugId: string) => void
  onSelectAllDrugs?: () => void
  onDeselectAllDrugs?: () => void
  // Report
...
export function DataPanel({
  ...
  const showCheckboxes = (simulationPhase === "ready" || simulationPhase === "complete") && !!onToggleDrugSelection
  const selectedCount = selectedDrugIds?.size ?? 0
      ...
      <div className="flex h-full flex-col">
        ...
        <div className="flex min-h-0 flex-1 flex-col">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] text-muted-foreground">
                  {drugs.some((d) => d.confidenceScore !== null)
                    ...
                    : `${drugs.length} from literature review`}
                </span>
              </div>
            )}
          </div>
                  ...
                  <TableRow className="border-border hover:bg-transparent">
                    {showCheckboxes && (
                      <TableHead className="h-8 w-10 text-center">
                        <Checkbox
                          checked={drugs.length > 0 && drugs.every(d => selectedDrugIds?.has(d.id))}
                          onCheckedChange={(checked) => {
                            if (checked) onSelectAllDrugs?.()
                            else onDeselectAllDrugs?.()
                          }}
                        />
                      </TableHead>
                    )}
                      ...
                      >
                        {showCheckboxes && (
                          <TableCell className="w-10 py-2.5 text-center">
                            <Checkbox
                              checked={selectedDrugIds?.has(drug.id) ?? false}
                              onCheckedChange={() => onToggleDrugSelection?.(drug.id)}
                        ...
                        <TableCell className="py-2.5">
                          <div className="flex items-center gap-1">
                            ...
                            {drug.trialStatusForTargets && (
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <FlaskConical className="h-3 w-3 shrink-0 text-amber-400" />
                                </TooltipTrigger>
                                <TooltipContent side="top" className="max-w-[250px]">
                                  <p className="text-xs">{drug.trialStatusForTargets}</p>
                                </TooltipContent>
                              </Tooltip>
                            )}
                          </div>
          ...
          {simulationPhase === "ready" && (
            <div className="border-t border-border p-4">
              ...
              >
                ...
                Run DiffDock on {selectedCount} drug{selectedCount !== 1 ? "s" : ""}
              </button>
              <p className="mt-2 text-center text-[10px] text-muted-foreground">
                Runs molecular docking on RunPod GPU for each selected candidate
              </p>
            </div>
          )}
          ...
          {simulationPhase === "running" && drugs && (
            (() => {
              const trackedTotal = Object.keys(drugSimStatuses).length
              const trackedComplete = Object.values(drugSimStatuses).filter(
                (s) => s === "complete"
              ).length
              const pctDone = trackedTotal > 0 ? (trackedComplete / trackedTotal) * 100 : 0
          ...
          {simulationPhase === "complete" && (
            <div className="flex flex-col gap-2 border-t border-border p-4">
              ...
              {onSearchSimilar && selectedCount > 0 && (
                <button
                  type="button"
                  onClick={onSearchSimilar}
                  className="flex w-full items-center justify-center gap-2 rounded-lg border border-primary/30 bg-primary/10 px-4 py-2.5 text-xs font-semibold text-primary transition-colors hover:bg-primary/20"
                >
                  <Search className="h-3.5 w-3.5" />
                  Search similar 3D compounds for {selectedCount} selected
                </button>
              )}
              {hasReport ? (
                <button
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-xs font-semibold text-primary-foreground transition-colors hover:bg-primary/90"
                >
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-xs font-semibold text-primary-foreground transition-colors hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {reportGenerating ? (
